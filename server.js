...
This is a simple static server for serving the applications files for assignment #2
...
http.createServer(function(request, response) {
    ...
    request.on("end", function() {
      ...
      if (request.method == "POST" && urlObj.pathname === "/fetchSong") {
        ...
        //a POST request to save a song
        let filePath = "songs/" + dataObj.fileName + ".txt"
        //check if file is already exist
        let alreadyExist = false
        for (title in songFiles) {
          if (songFiles[title] == filePath)
            alreadyExist = true;
        }

        fs.writeFile(filePath, dataObj.fileContent, function(err, data) {
          if (err) {
            returnObj.text = "FILE WRITE ERROR"
            response.writeHead(200, {"Content-Type": MIME_TYPES["json"]})
            response.end(JSON.stringify(returnObj))
          } else {
            if (!alreadyExist) {
              songFiles[dataObj.fileName] = filePath
            }
            returnObj.text = "FILE SAVED SUCCESSFULLY"
            returnObj.content = dataObj.fileContent
